...
.auth-particle{--size:5px;--x:50%;--delay:0s;--duration:18s;position:absolute;left:var(--x);bottom:-24px;width:var(--size);height:var(--size);border-radius:50%;background:var(--particle-color,var(--text-2));opacity:0;animation:particleRise var(--duration) linear var(--delay) infinite;will-change:transform,opacity}
...
for(let i=0;i<24;i++){const p=document.createElement('span');p.className='auth-particle';if(Math.random()<.42)p.classList.add('accent');if(Math.random()<.3)p.classList.add('ring');if(i%5===0)p.classList.add('glow');p.style.setProperty('--size',`${2.5+Math.random()*5}px`);p.style.setProperty('--x',`${Math.random()*100}%`);p.style.setProperty('--delay',`${-Math.random()*22}s`);p.style.setProperty('--duration',`${16+Math.random()*13}s`);p.style.setProperty('--drift',`${-55+Math.random()*110}px`);layer.appendChild(p)}
page.insertBefore(layer,page.querySelector('.auth-card'))
let pointerFrame=0
page.addEventListener('pointermove',e=>{if(pointerFrame)return;pointerFrame=requestAnimationFrame(()=>{pointerFrame=0;const r=page.getBoundingClientRect();const x=(e.clientX-r.left-r.width/2)/r.width;const y=(e.clientY-r.top-r.height/2)/r.height;layer.style.transform=`translate3d(${x*-16}px,${y*-10}px,0)`})},{passive:true})
