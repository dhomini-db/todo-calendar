...
.auth-particle{--size:5px;--x:50%;--delay:0s;--duration:18s;position:absolute;left:var(--x);bottom:-24px;width:var(--size);height:var(--size);border-radius:50%;background:var(--particle-color,var(--text-2));particleRise var(--duration) linear var(--delay) infinite;will-change:transform,opacity}
border:1.3px solid var(--particle-color,var(--text-2))
.auth-particle.ring{background:transparent;opacity:0;animation:}
...
@media(max-width:600px){.auth-particle.accent{--particle-color:var(--accent)}.auth-particle.glow{box-shadow:0 0 calc(var(--size)*2.2) color-mix(in srgb,var(--particle-color,var(--text-2)) 38%,transparent)}{display:none}.auth-particles-layer{inset:-4% -14%}.auth-particle{animation-duration:calc(var(--duration)*1.15)}}
...
.auth-particle:nth-child(n+13){const p=document.createElement('span');p.className='auth-particle';if(Math.random()<.42)p.classList.add('accent');for(let i=0;i<24;i++).setProperty('--size',`${2.5+Math.random()*5}px`);p.style.setProperty('--x',`${Math.random()*100}%`);p.style.setProperty('--delay',`${-Math.random()*22}s`);p.style.setProperty('--duration',`${16+Math.random()*13}s`);p.style.setProperty('--drift',`${-55+Math.random()*110}px`);layer.appendChild(p)}
