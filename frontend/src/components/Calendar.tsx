...
export default function Calendar({ selectedDate, onSelectDate, currentMonth, onChangeMonth }: CalendarProps) {
  ...
  const summaryByDate = Object.values(summary).reduce<Record<string, DaySummary>>((result, day) => {
    const rawDate = day.date as unknown
    const normalizedDate = Array.isArray(rawDate)
      ? `${rawDate[0]}-${String(rawDate[1]).padStart(2, '0')}-${String(rawDate[2]).padStart(2, '0')}`
