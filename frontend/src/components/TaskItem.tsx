...
export default function TaskItem({ task, date }: TaskItemProps) {
  ...
  return (
    <div className={`task-item ${isPositive ? 'task-positive' : 'task-negative'} ${task.completed ? 'done' : ''} ${editing ? 'editing-full' : ''}`}>
      ...
      {!editing && <div className="task-actions">{confirmDelete ? <div className="delete-confirm"><button className="confirm-yes" onClick={() => remove.mutate(task.id)}>{t('common.delete')}</button><button className="confirm-no" onClick={() => setConfirmDelete(false)}>{t('common.cancel')}</button></div> : <>
